...
shares *attack_evaland(shares *x, shares *y, u8 ptx)
{
    ...
    for(i = 0; i < 2 + N; i++)
    {
        trace[ptx][my_index++] = x_cooperands[i];
        trace[ptx][my_index++] = y_cooperands[i];
    }
    return s;
}
...
{
    srand(time(NULL));
    u8 perm1[64], perm1_inv[64], perm2[64], perm2_inv[64];
    generatePermutation64(perm1, perm1_inv);
    generatePermutation64(perm2, perm2_inv);
    u8 tmpk[16];
    generatePermutation(key, tmpk);
    ...
    for(i = 0; i < 4096; i++)
    {
        ...
        tmp = (perm1[tmp >> 6] << 6) | perm2[tmp & 0x3f];

        tmp = (((tmp >> 8) ^ ((tmp >> 4) & 0xf)) << 4) | (tmp & 0xf);
        // in[byteth] = i;
        in[byteth] = tmp;
        encode_ptx(in, x);
        memmove(y, x, sizeof(shares*) * 128);
